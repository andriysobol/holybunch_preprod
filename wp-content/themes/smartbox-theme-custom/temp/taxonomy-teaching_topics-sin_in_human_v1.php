...
/* if override image not empty , display default image as background.
if(empty(get_query_var('term'))){
    $taxonomy_topic = get_query_var('taxonomy_topics');        
}else{ 
    $taxonomy_topic = get_query_var('term');
}*/

$taxonomy_category = get_query_var('oxy_content_category');
$data_image = '';
$override_image = get_taxonomy_image("teaching_topics", $taxonomy_topic);
if (!empty($override_image) && $taxonomy_category == 'music') {
    $data_image = 'data-background="url(' . $override_image . ') no-repeat top"';
}
    ...
    $args = array(
        ...
        'tax_query' => array(
            array(
                ...
                'field' => 'slug',
                'terms' => get_query_var('term'), // should work without a slug, try it both ways...and use a variable, don't hardcode
                'include_children' => false,
            ),
            array(
                'taxonomy' => 'oxy_content_category', // slug for desired tag goes here
                'field' => 'slug',
                'terms' => get_query_var('oxy_content_category'), // should work without a slug, try it both ways...and use a variable, don't hardcode
                'include_children' => false,
            )
        )
    );
    ...
    $args = array(
        ...
        'tax_query' => array(
            array(
                'taxonomy' => 'teaching_topics', // slug for desired tag goes here
                'field' => 'slug',
                'terms' => $taxonomy_topic, // should work without a slug, try it both ways...and use a variable, don't hardcode
                'include_children' => false,
            )
        )
    );
